...
from PyQt6.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QCheckBox, QToolButton, QSizePolicy, QApplication, QDialog)
...
class ImageComparisonDialog(QDialog):
    """Dialog to display original and encoded images side by side"""
    
    def __init__(self, original_path, encoded_path, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Before & After Comparison")
        self.setModal(True)
        self.setMinimumSize(1200, 800)  # Much larger size for better visibility
        self.resize(1200, 800)  # Set initial size to be larger
        
        # Create main layout with reduced top margin for higher title position
        layout = QVBoxLayout()
        layout.setContentsMargins(30, 10, 30, 20)  # Reduced top margin to position title higher
        layout.setSpacing(15)  # Balanced spacing between elements
        layout.setAlignment(Qt.AlignmentFlag.AlignCenter)  # Center all content
        self.setLayout(layout)
        
        # Title with larger font and positioned higher
        title_label = QLabel("Image Comparison")
        title_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        title_label.setStyleSheet("""
            QLabel {
                font-size: 28px;  /* Increased from 24px for bigger text */
                font-weight: bold;
                color: #45edf2;
                margin: 0px;  /* No margins for title */
                padding: 8px;  /* Reduced padding to position higher */
            }
        """)
        layout.addWidget(title_label)
        
        # Images container with balanced spacing and centered alignment
        images_container = QHBoxLayout()
        images_container.setSpacing(20)  # Spacing between left and right sections
        images_container.setAlignment(Qt.AlignmentFlag.AlignCenter)  # Center the images container
        
        # Original image section
        original_section = QVBoxLayout()
        original_section.setSpacing(8)  # Balanced spacing between label and image
        original_section.setAlignment(Qt.AlignmentFlag.AlignCenter)  # Center content
        original_label = QLabel("BEFORE")
        ...
        self.original_image_label.setStyleSheet("""
            QLabel {
                border: 3px solid rgba(69,237,242,0.8);  /* Thicker, more visible border */
                border-radius: 12px;  /* Larger border radius */
                background-color: #1a1a2e;
                padding: 5px;  /* Reduced padding to make images larger within boxes */
                min-width: 500px;  /* Keep same box size */
                min-height: 400px;  /* Keep same box size */
            }
        """)
        original_section.addWidget(self.original_image_label)
        
        # Encoded image section
        encoded_section = QVBoxLayout()
        encoded_section.setSpacing(8)  # Balanced spacing between label and image
        encoded_section.setAlignment(Qt.AlignmentFlag.AlignCenter)  # Center content
        encoded_label = QLabel("AFTER")
        encoded_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        encoded_label.setStyleSheet("""
            QLabel {
                font-size: 18px;  /* Increased from 14px */
                font-weight: bold;
                color: #e8e8fc;
        ...
        """)
        encoded_section.addWidget(encoded_label)
        
        self.encoded_image_label = QLabel()
        self.encoded_image_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.encoded_image_label.setStyleSheet("""
            QLabel {
                border: 3px solid rgba(69,237,242,0.8);  /* Thicker, more visible border */
                border-radius: 12px;  /* Larger border radius */
                background-color: #1a1a2e;
        ...
        """)
        encoded_section.addWidget(self.encoded_image_label)
        
        images_container.addLayout(original_section)
        images_container.addLayout(encoded_section)
        layout.addLayout(images_container)
        
        # Close button - smaller size
        close_button = QPushButton("Close")
        close_button.setMinimumSize(80, 35)  # Smaller button size
        close_button.setStyleSheet("""
            QPushButton {
                background: rgba(69,237,242,0.1);
                color: #45edf2;
                border: 2px solid rgba(69,237,242,0.8);
                padding: 6px 12px;  /* Reduced padding to accommodate bigger text */
        ...
        self.load_images(original_path, encoded_path)
    
    def load_images(self, original_path, encoded_path):
        """Load and display the original and encoded images"""
        try:
            # Load original image
            if os.path.exists(original_path):
                ...
                if not original_pixmap.isNull():
                    ...
                    scaled_original = original_pixmap.scaled(350, 350, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
                    self.original_image_label.setPixmap(scaled_original)
                else:
                    self.original_image_label.setText("Failed to load original image")
            else:
                self.original_image_label.setText("Original image not found")
            
            # Load encoded image
            if os.path.exists(encoded_path):
                encoded_pixmap = QPixmap(encoded_path)
                if not encoded_pixmap.isNull():
                    # Scale image to fit in dialog while maintaining aspect ratio
                    scaled_encoded = encoded_pixmap.scaled(350, 350, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
                    self.encoded_image_label.setPixmap(scaled_encoded)
                else:
                    self.encoded_image_label.setText("Failed to load encoded image")
            else:
                self.encoded_image_label.setText("Encoded image not found")
                
        except Exception as e:
            print(f"Error loading images: {e}")
            self.original_image_label.setText("Error loading original image")
            self.encoded_image_label.setText("Error loading encoded image")
...
class StegaEncodeWindow(QMainWindow):
    ...
    def is_start_location_ready(self) -> bool:
        """Check if start location is selected based on media type"""
        if self.media_type == 'image':
            return self.start_xy is not None
        elif self.media_type == 'audio':
            return self.start_sample is not None
        elif self.media_type == 'video':
            return self.video_start is not None and self.video_start != (0, 0, 0)
        return False
    ...
    def hide_message(self):
        ...
        if ok:
            ...
            compare_btn = msg.addButton('Compare Images', QMessageBox.ButtonRole.ActionRole)
            ...
            # Set specific button sizes to prevent text cutoff
            open_btn.setMinimumWidth(90)  # Smaller for "Open file"
            compare_btn.setMinimumWidth(140)  # Larger for "Compare Images" 
            close_btn.setMinimumWidth(80)  # Smaller for "Close"
            ...
            elif msg.clickedButton() == compare_btn and self.media_type == 'image':
                # Show image comparison popup for image steganography
                original_path = self.media_drop_widget.media_path
                encoded_path = self.machine.output_path
                if original_path and encoded_path and os.path.exists(original_path) and os.path.exists(encoded_path):
                    comparison_dialog = ImageComparisonDialog(original_path, encoded_path, self)
    ...
    def update_capacity_panel(self):
        ...
        self.cap_dims.setText(f"{w}x{h}x{channels}")
        self.cap_lsb.setText(f"{lsb}")
